...
class PetRepository:
    ...
    def _not_deleted(self) -> Select:
        return select(Pet).where(Pet.is_deleted.is_(False))

    # ─── Reads ────────────────────────────────────────────────────────────────

    async def get_by_id(self, pet_id: str) -> Optional[Pet]:
        """Return the pet if it exists and is not soft-deleted."""
        result = await self.session.execute(
            self._not_deleted().where(Pet.id == pet_id)
        )
    ...
    async def add(self, pet: Pet) -> Pet:
        """
        Stage a new pet for insertion and flush so DB-side constraint checks
        (e.g. the unique microchip index) run now, without committing — the
        request-scoped session owns the commit.
        """
    ...
    async def save(self, pet: Pet) -> Pet:
        """Persist in-place changes to an already-tracked pet."""
        self.session.add(pet)
        await self.session.flush()
        await self.session.refresh(pet)
        return pet
